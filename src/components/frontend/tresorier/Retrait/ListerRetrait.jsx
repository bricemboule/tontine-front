...
export default function ListerRetrait(){

   const {getRetraits, retraits,supprimerRetrait} = useContext(RoleContext);
        ...
        <div className="content-wrapper">
            ...
            <div className="d-flex space-between p-3 bg-gray-light">
                <div className="d-flex flex-wrap gap-2">
                    <button type="button" className="btn btn-success"  style={{padding:"10px"}}>
                        <Link to='/tresorier/retrait/ajouter' className="text-decoration-none text-white">
            ...
            <table id="paginationNumbers" className= "table"   width="100%">
                <thead >
                        ...
                        <th scope="col" className="text-center p-3" style={{width:"400px"}}>Type retrait</th>
                ...
                <tbody>
                    {retraits.map((retrait)=> {
                        return(
                            <tr key={retrait.id} style={{height:"20%"}}>
                            <th scope="row" className="p-3">
                                <div className="form-check">
                                <input className="form-check-input" type="checkbox" value="" id="flexCheckDefault" />
                                </div>
                            </th>
                            <td className="text-center p-3">{retrait.id}</td>
                            <td className="text-center p-3">{retrait.user.nom}</td>
                            <td className="text-center p-3">{retrait.montant}</td>
                            <td className="text-center p-3">{retrait.type_retrait.intitule}</td>             
                            <td className="text-center p-3">{retrait.seance.dateSeance}</td>
                         
                            
                            <td className="text-center p-3">
                            
                                <button className="btn btn-link btn-floating" ><Link to={`/tresorier/retrait/${retrait.id}/modifier`}> <i className="far fa-edit text-green"></i></Link> </button>
                                <button className="btn btn-link btn-floating" onClick={()=>supprimerRetrait(retrait.id)}> <i className="far fa-trash-alt text-red"></i></button>
                            </td>
                        </tr>
                        ); }
                    )}
